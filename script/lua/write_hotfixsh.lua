...
if "%load_mods%" == "" (
    echo please format make\script\hotfix.bat load_mods.lua ***_m ***_m
    exit /b 1
)
...
echo params=%params%
set hotfix_url=""
for /f "delims=" %%i in ('{lua_path} {skynet_fly_path}\script\lua\console.lua {skynet_fly_path} {svr_name} %load_mods% hotfix %serverid:~0,9% %params%') do (
    set hotfix_url=%%i
)

echo %hotfix_url:~1,-1%

for /f "delims=" %%i in ('curl -s -X GET "%hotfix_url:~1,-1%"') do (
    echo %%i
    {lua_path} {skynet_fly_path}\script\lua\console.lua {skynet_fly_path} {svr_name} %load_mods% handle_hotfix_result %%i
    goto end
)
...
:end
    ...
    local file_path = bat_path .. 'hotfix.bat'
