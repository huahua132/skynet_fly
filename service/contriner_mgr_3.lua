...
function CMD.load_module(source,module_name,launch_num)
	...
	if watch_map then
		for src in pairs(watch_map) do
			local session = skynet_send(src,'lua','watchrsp',module_name,id_list)
			if not session then
				skynet.error("send invaild address ",src)
			end
		end
	end
...
function CMD.watch(source,module_name)
    assert(module_name,'not module_name')
...
skynet.start(function()
	...
	skynet.dispatch('lua',function(session,source,cmd,...)
		...
		skynet_ret(skynet_pack(f(source,...)))
	end)
end)