...
local function monitor(t,key)
	while not IS_CLOSE do
		...
		local id_list,version = skynet.call('.contriner_mgr','lua','watch',SELF_ADDRESS,key,old_version)
		t[key] = id_list
...
local g_mod_svr_ids_map = setmetatable({},{__index = function(t,key)
	t[key],g_mod_svr_version_map[key] = skynet.call('.contriner_mgr','lua','query',SELF_ADDRESS,key)
