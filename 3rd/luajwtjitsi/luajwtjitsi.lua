...
local pri_mt = {__index = function(t,key)
	local v = pkey.read(key,true,'pem')
	if not v then
		return nil
	end
	t[key] = v
...
local g_pri_map = setmetatable({},pri_mt)


local cert_mt = {__index = function(t,key)
	local v = x509.read(key,'pem')
	if not v then
		return nil
	end
	t[key] = v
...
local g_cert_map = setmetatable({},cert_mt)

local function signRS (data, key, algo)
	local prikey = g_pri_map[key]
	if prikey == nil then
		return nil, 'Not a private PEM key'
	end

	local sha = openssl.digest.new(algo)
	sha:update(data)
	local sum = sha:final(true)

	local sign_ctx = prikey:ctx()
	sign_ctx:sign_init()
	
	local sign = true 
	sign = sign and sign_ctx:ctrl("rsa_padding_mode","pss")
	sign = sign and sign_ctx:ctrl("digest", algo)
