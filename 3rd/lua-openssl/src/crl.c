/***
x509.crl module to mapping `X509_CRL` to lua object, creates and  processes CRL file in DER or PEM
format.
...
const char *
openssl_i2s_revoke_reason(int reason)
{
  int i;
...
int
openssl_s2i_revoke_reason(const char *s)
{
  int i;
...
static int
reason_get(lua_State *L, int reasonidx)
{
  int reason = 0;
...
static int
openssl_x509_revoked_get_reason(X509_REVOKED *revoked)
{
  int              crit = 0;
  int              reason = 0;
...
static X509_REVOKED *
create_revoked(const BIGNUM *bn, time_t t, int reason)
{
...
Note if not give evp_pkey, will create a new x509_crl object,if give will generate a signed x509_crl
object.
@function new
...
static int openssl_crl_new(lua_State *L)
{
  ...
  for (i = 1; ret == 1 && i <= n; i++) {
    if (i == 1) {
      luaL_argcheck(
        L, lua_istable(L, 1), 1, "must be table contains rovked entry table{reason,time,sn}");
      if (lua_rawlen(L, i) > 0) {
        int j, m;
        m = lua_rawlen(L, i);
    ...
    if (i == 3) {
      ...
      luaL_argcheck(
        L, X509_check_private_key(cacert, capkey) == 1, 3, "evp_pkey not match with x509 in #2");
    }
  }
  ...
  if (ret == 1) {
    ...
    if (ret == 1) ret = X509_CRL_set1_nextUpdate(crl, ntm);
    ASN1_TIME_free(ltm);
    ASN1_TIME_free(ntm);
  }
  if (cacert && capkey && md) {
...
*/
static int
openssl_crl_reason(lua_State *L)
{
  int                       i;
...
*/
static int
openssl_crl_extensions(lua_State *L)
{
  X509_CRL *crl = CHECK_OBJECT(1, X509_CRL, "openssl.x509_crl");
  if (lua_isnone(L, 2)) {
    ...
    STACK_OF(X509_EXTENSION) *exts
      = (STACK_OF(X509_EXTENSION) *)openssl_sk_x509_extension_fromtable(L, 2);
    int i, n;
...
static int openssl_crl_issuer(lua_State *L)
{
  ...
  if (lua_isnone(L, 2)) {
    ...
    luaL_argcheck(L, xn || x, 2, "only accept openssl.x509 or openssl.x509_name object");
    if (xn) {
...
int openssl_crl_sign(lua_State *L)
{
  ...
  if (ret == 1) {
    ...
    if (ret == 1) ret = X509_CRL_sign(crl, key, md);
  }
  return openssl_pushresult(L, ret);
}
...
static int openssl_crl_parse(lua_State *L)
{
  ...
  /* hash as used in CA directories to lookup cert by subject name */
  {
    ...
    snprintf(
      buf, sizeof(buf), "%08lx", X509_NAME_hash_ex(X509_CRL_get_issuer(crl), NULL, NULL, NULL));
#else
...
static int openssl_crl_export(lua_State *L)
{
  ...
  luaL_argcheck(L, fmt == FORMAT_DER || fmt == FORMAT_PEM, 2, "only accept der or pem");
...
static int
openssl_revoked_free(lua_State *L)
{
  X509_REVOKED *revoked = CHECK_OBJECT(1, X509_REVOKED, "openssl.x509_revoked");
