...
@function new
@tparam table certs array of x509 objects, all x509 object will add to store, certs can be empty but
not nil
...
openssl_xstore_new(lua_State *L)
{
  ...
  if (!lua_isnoneornil(L, 1)) {
    ...
    for (i = 0; i < n; i++) {
      ...
      lua_rawgeti(L, 1, i + 1);
      luaL_argcheck(
        L, auxiliar_getclassudata(L, "openssl.x509", -1), 1, "only contains x509 object");
...
*/
void
openssl_xstore_free(X509_STORE *ctx)
{
  /* hack openssl bugs */
...
*/
static int
openssl_xstore_add_lookup(lua_State *L)
{
  X509_STORE        *ctx = CHECK_OBJECT(1, X509_STORE, "openssl.x509_store");
...
*/
static int
openssl_xstore_depth(lua_State *L)
{
  X509_STORE *ctx = CHECK_OBJECT(1, X509_STORE, "openssl.x509_store");
...
*/
static int
openssl_xstore_flags(lua_State *L)
{
  X509_STORE *ctx = CHECK_OBJECT(1, X509_STORE, "openssl.x509_store");
...
*/
static int
openssl_xstore_purpose(lua_State *L)
{
  X509_STORE *ctx = CHECK_OBJECT(1, X509_STORE, "openssl.x509_store");
...
*/
static int
openssl_xstore_trust(lua_State *L)
{
  X509_STORE *ctx = CHECK_OBJECT(1, X509_STORE, "openssl.x509_store");
...
*/
static int
openssl_xstore_load(lua_State *L)
{
  X509_STORE *ctx = CHECK_OBJECT(1, X509_STORE, "openssl.x509_store");
  ...
  if (file || dir) {
    ...
    if (file != NULL) ret = X509_STORE_load_file(ctx, file);
    if (ret == 1 && dir != NULL) ret = X509_STORE_load_path(ctx, dir);
#else
...
*/
static int
openssl_xstore_add(lua_State *L)
{
  X509_STORE *ctx = CHECK_OBJECT(1, X509_STORE, "openssl.x509_store");
  int         n = lua_gettop(L);
  int         i;
  ...
  for (i = 2; i <= n; i++) {
    if (lua_istable(L, i)) {
      ...
      luaL_argerror(
        L, i, "only accept x509 or x509_crl object or table with x509 or x509_crl object");
    }
    if (ret == 0) break;
  }
...
int
openssl_register_xstore(lua_State *L)
{
