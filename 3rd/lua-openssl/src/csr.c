/***
x509.req module to mapping `X509_REQ` to lua object, creates and processes certificate requests
...
static int openssl_csr_read(lua_State *L)
{
  ...
  if (csr) PUSH_OBJECT(csr, "openssl.x509_req");
  return csr ? 1 : openssl_pushresult(L, 0);
}

/***
create or generate a new x509_req object.
Note if not give evp_pkey, will create a new x509_req object,or will generate a signed x509_req
object.
@function new
...
copy_extensions(X509 *x, X509_REQ *req, int override)
{
  ...
  for (i = 0; i < sk_X509_EXTENSION_num(exts); i++) {
    ...
    if (!X509_add_ext(x, ext, -1)) goto end;
  }
...
X509_REQ_to_X509_ex(X509_REQ *r, int days, EVP_PKEY *pkey, const EVP_MD *md, X509 **x)
{
  ...
  if ((res = X509_set_subject_name(ret, xn)) != 1) goto err;
  if ((res = X509_set_issuer_name(ret, xn)) != 1) goto err;
  ...
  if ((res = X509_set1_notBefore(ret, notBefore)) != 1) goto err;
  ...
  if ((res = X509_set1_notAfter(ret, notAfter)) != 1) goto err;
  ASN1_TIME_free(notAfter);
  notAfter = NULL;

  if ((res = copy_extensions(ret, r, 1)) != 1) goto err;
  ...
  if ((res = X509_sign(ret, pkey, md)) == 0) goto err;
  res = 1;
...
static int openssl_csr_export(lua_State *L)
{
  ...
  ret = fmt == FORMAT_PEM ? PEM_write_bio_X509_REQ(out, csr) : i2d_X509_REQ_bio(out, csr);
  if (ret == 1) {
    BUF_MEM *bio_buf;
...
static int
set_csr_pubkey(X509_REQ *csr, EVP_PKEY *pkey)
{
...
static int openssl_csr_sign(lua_State *L)
{
  ...
  if (auxiliar_getclassudata(L, "openssl.evp_pkey", 2)) {
    ...
    if (ret == 1) ret = X509_REQ_sign(csr, pkey, md);
    return openssl_pushresult(L, ret);
