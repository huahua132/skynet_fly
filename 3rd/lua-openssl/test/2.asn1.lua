...
local function asn1parse(s, off, last, indent)
  ...
  if cons then
    ...
    if first then
      print(string.format("%sVAL:%s", string.rep(tab, indent + 1), hex(string.sub(s, start, stop))))
    end
...
function TestAsn1_2.testParse()
  ...
  lu.assertErrorMsgEquals(
    "2.asn1.lua:23: bad argument #2 to 'get_object' (start out of length of asn1 string)",
    asn1parse,
    ss,
    0
  )
  ...
  lu.assertErrorMsgEquals(
    "2.asn1.lua:23: bad argument #3 to 'get_object' (stop out of length of asn1 string)",
    asn1parse,
    ss,
    1,
    #ss + 1
